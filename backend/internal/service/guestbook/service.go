...
func (s *Service) ListPublic(ctx context.Context, eventID uuid.UUID) ([]model.GuestbookMessage, error) {
	return s.guestbookRepo.ListByEvent(ctx, eventID, true)
}

func (s *Service) ListAll(ctx context.Context, userID uuid.UUID, eventID uuid.UUID) ([]model.GuestbookMessage, error) {
	event, err := s.eventRepo.GetByID(ctx, eventID)
	if err != nil || event == nil {
		return nil, errors.ErrNotFound
	}
	...
	return s.guestbookRepo.ListByEvent(ctx, eventID, false)
}
