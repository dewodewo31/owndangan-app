          ...
          {howItWorks.map((step, index) => (
            <StaggerItem key={step.step} className="h-full">
              <div className="relative h-full">
                ...
                <div className="relative bg-white rounded-2xl border border-border p-8 h-full text-center shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-primary/10 hover:border-primary/40">
                  <div
                    className={cn(
                      "w-20 h-20 mx-auto rounded-2xl flex items-center justify-center mb-6 relative",
                      "bg-gradient-to-br from-plum to-rosegold shadow-lg shadow-primary/25"
                    )}
                  >
                    <span className="text-2xl font-bold text-white">{step.step}</span>
                    <span className="absolute -top-2 -right-2 h-5 w-5 rounded-full bg-white border-2 border-primary" />
                  </div>
