...
export default function AuthLayout({ children, side = "login", className }: AuthLayoutProps) {
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      {/* Left: Brand panel */}
      <div className="hidden lg:flex flex-col justify-between bg-gradient-to-br from-indigo-600 via-violet-700 to-fuchsia-700 p-12 text-white relative overflow-hidden">
        ...
        <div className="relative max-w-md">
          ...
          <h2 className="text-4xl font-bold leading-tight tracking-tight">
            {side === "register" ? (
              <>
                Mulai buat undangan
                <span className="block text-indigo-200">impian kalian.</span>
              </>
            ) : (
              <>
                Undangan digital
                <span className="block text-indigo-200">yang tak terlupakan.</span>
              </>
            )}
          </h2>
            ...
            {highlights.map((item) => (
              <div key={item.title} className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-indigo-100/80 mt-0.5">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className={cn("flex items-center justify-center px-4 sm:px-6 lg:px-12 py-12", className)}>
        <div className="w-full max-w-md">
          <Link href="/" className="lg:hidden inline-flex items-center gap-2 mb-8">
            <span className="inline-flex items-center justify-center h-9 w-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white">
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
            </span>
